...
interface TreeRoot extends Node {
  children: (Node | EmbedNode)[];
}

function transformed(md: string): TreeRoot {
  const processor = unified().use(remarkParse).use(remarkEmbeds);
  const tree = processor.parse(md);
  return processor.runSync(tree) as TreeRoot;
}
...
describe('remarkEmbeds', () => {
  it('converts bare YouTube URL on its own line into youtube node', () => {
    ...
    const embed = tree.children.find((n) => n.type === 'embed') as EmbedNode;
  ...
  it('recognizes youtu.be short links', () => {
    ...
    const embed = tree.children.find((n) => n.type === 'embed') as EmbedNode | undefined;
    expect(embed?.provider).toBe('youtube');
    expect(embed?.props.videoId).toBe('dQw4w9WgXcQ');
  });
  ...
  it('leaves URL inside a sentence untouched', () => {
    ...
    expect(tree.children.some((n) => n.type === 'embed')).toBe(false);
  });
});
