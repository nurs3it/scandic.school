...
import { ArrowRight, Beaker, Calculator, Palette, Languages, Atom, Cpu, Lightbulb } from 'lucide-react';
...
export async function ProgramsPageContent() {
  ...
  const stemIcons = [
    { icon: Beaker, label: 'Science', delay: '0s' },
    { icon: Calculator, label: 'Technology', delay: '0.5s' },
    { icon: Atom, label: 'Engineering', delay: '1s' },
    { icon: Cpu, label: 'Math', delay: '1.5s' },
  ];
  ...
  return (
    <>
          ...
          <ScrollReveal delay={0.3}>
            <div className="mt-8 inline-flex items-center gap-3 bg-white/10 backdrop-blur-sm border border-white/20 rounded-full px-5 py-2.5">
              ...
              <span className="text-white/90 text-sm font-medium">
                <span className="text-primary font-bold text-lg mr-1">4</span>
                {locale === 'en' ? 'core subject areas' : locale === 'kk' ? 'негізгі пән бағыты' : 'основных направления'}
              </span>
            </div>
          </ScrollReveal>
        </div>
      ...
      {/* ===== SUBJECTS — IMMERSIVE ALTERNATING LAYOUT ===== */}
        ...
        <div className="container mx-auto px-4 md:px-8">
          <ScrollReveal>
            <div className="text-center mb-14 md:mb-20">
              ...
              <h2 className="text-3xl md:text-5xl font-bold gradient-text mb-5">{t.subjectsTitle as string}</h2>
              <p className="text-lg text-gray-500 max-w-2xl mx-auto leading-relaxed">{t.subjectsSubtitle as string}</p>
            </div>
          </ScrollReveal>
          ...
          <div className="max-w-6xl mx-auto space-y-8 md:space-y-12">
            {subjects.map((subject, index) => {
              ...
              return (
                <ScrollReveal key={index} direction={isReversed ? 'right' : 'left'} delay={0.1}>
                  <div className={`group relative grid grid-cols-1 lg:grid-cols-5 gap-0 rounded-2xl overflow-hidden border ${subject.accentBorder} ${subject.accentHover} bg-white hover:shadow-2xl hover:shadow-gray-200/80 transition-all duration-500`}>
                    {/* Icon side */}
                    <div className={`lg:col-span-2 relative bg-gradient-to-br ${subject.accentBg} p-8 md:p-12 flex items-center justify-center min-h-[220px] overflow-hidden ${isReversed ? 'lg:order-2' : ''}`}>
                      {/* Large background number */}
                      <span className="absolute -top-4 -left-2 text-white/[0.05] text-[10rem] font-bold leading-none select-none">
                        {subject.number}
                      </span>
                      {/* Decorative circles */}
                      ...
                      <div className="absolute bottom-6 left-6 w-14 h-14 bg-white/[0.07] rounded-full group-hover:scale-125 transition-transform duration-700" />

                      <div className="h-24 w-24 md:h-28 md:w-28 relative z-10 opacity-90 group-hover:scale-110 transition-transform duration-500">
                    ...
                    <div className={`lg:col-span-3 p-8 md:p-10 lg:p-12 flex flex-col justify-center ${isReversed ? 'lg:order-1' : ''}`}>
                      ...
                      <div className="flex items-center gap-3 mb-4">
                        ...
                        <div className={`w-10 h-1 ${subject.accent} rounded-full opacity-60`} />
                      </div>

                      <h3 className="text-xl md:text-2xl font-bold text-secondary mb-3">{data.title as string}</h3>
                      <p className="text-gray-600 leading-relaxed mb-6 text-[15px]">{data.description as string}</p>
                      ...
                      {tags.length > 0 && (
                        <div className="flex flex-wrap gap-2">
                          {tags.map((tag, tagIndex) => (
                            <span
                              key={tagIndex}
                              className="inline-flex items-center px-3 py-1.5 rounded-full text-xs font-medium bg-gray-50 text-gray-600 border border-gray-200 hover:border-gray-300 hover:bg-gray-100 transition-colors cursor-default"
                            >
                              {tag}
                            </span>
                          ))}
        ...
        <div className="container mx-auto px-4 md:px-8 relative z-10">
          <ScrollReveal>
            <div className="max-w-3xl mx-auto text-center mb-12">
              <span className="inline-flex items-center gap-2 mb-4">
                <span className="w-8 h-[2px] bg-primary rounded-full" />
                <span className="text-primary text-sm font-semibold uppercase tracking-[0.2em]">{t.stemSubtitle as string}</span>
              </span>
              <h2 className="text-3xl md:text-4xl lg:text-5xl font-bold text-white leading-tight mb-6">{t.stemTitle as string}</h2>
              <p className="text-white/70 text-base md:text-lg leading-relaxed">{t.stemDescription as string}</p>
            </div>
          </ScrollReveal>
          ...
          <div className="max-w-2xl mx-auto grid grid-cols-2 md:grid-cols-4 gap-6">
            {stemIcons.map((item, index) => {
              const StemIcon = item.icon;
              return (
                <ScrollReveal key={index} delay={0.2 + index * 0.1}>
                  <div className="group flex flex-col items-center gap-3 cursor-default">
                    <div className="w-20 h-20 rounded-2xl bg-white/[0.06] border border-white/10 flex items-center justify-center group-hover:bg-primary/20 group-hover:border-primary/30 group-hover:scale-110 transition-all duration-300">
                      <StemIcon className="h-9 w-9 text-white/60 group-hover:text-primary transition-colors duration-300" />
                    </div>
      ...
      <section className="border-y border-gray-200 bg-white">
        <div className="container mx-auto px-4 md:px-8">
          <div className="flex items-center gap-6 py-4 overflow-x-auto">
            <span className="text-secondary font-bold italic text-lg whitespace-nowrap">{t.navLabel as string}</span>
            <div className="w-px h-6 bg-gray-300" />
            <Link href="/about" className="text-sm text-gray-500 hover:text-secondary transition-colors whitespace-nowrap">{nav.about}</Link>
            <Link href="/structure" className="text-sm text-gray-500 hover:text-secondary transition-colors whitespace-nowrap">{nav.structure ?? 'Structure'}</Link>
            <Link href="/staff" className="text-sm text-gray-500 hover:text-secondary transition-colors whitespace-nowrap">{nav.staff}</Link>
          </div>
        </div>
      </section>
