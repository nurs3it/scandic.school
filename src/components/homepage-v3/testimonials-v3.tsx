      ...
      <div className="container mx-auto px-4">
        ...
        <div className="grid md:grid-cols-3 gap-6 mt-12">
          {t.items.map((item, idx) => (
            <div key={idx} className="bg-brand-teal-50 rounded-2xl p-6 flex flex-col">
              <div className="flex gap-1 mb-3">
                {Array.from({ length: 5 }).map((_, i) => (
                  <Star
                    key={i}
                    className="w-4 h-4 fill-brand-orange-400 text-brand-orange-400"
                  />
                ))}
              </div>
              <p className="text-brand-navy-900 leading-relaxed flex-1 mb-6">{item.quote}</p>
              <div className="flex items-center gap-3 pt-4 border-t border-brand-teal-100">
                <Image
                  src={item.image}
                  alt={item.name}
                  width={48}
